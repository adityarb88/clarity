...
/**
 * Credit: https://github.com/angular/angular/blob/d156e72ad7dfd95e14c41868c6e304e3f8af459c/packages/common/src/i18n/format_date.ts
 * This file is copied form the link mentioned above with slight modifications. Unused parts are commented out.
 */

import {FormStyle, FormatWidth, NumberSymbol, Time, TranslationWidth, getLocaleDateFormat, getLocaleDayNames, getLocaleDayPeriods, getLocaleEraNames, getLocaleExtraDayPeriodRules, getLocaleExtraDayPeriods, getLocaleId, getLocaleMonthNames, getLocaleNumberSymbol} from "@angular/common";
 ...
 */
/**
 * Changing the function slightly from
 * export function formatDate(date: Date, format: string, locale: string, timezone?: string): string {
 * to below. We do not handle timeZone as of now and we need the format to be ShortDate always
 */
export function formatUserDate(date: Date, locale: string, format = "shortDate"): string {
    ...
    parts.forEach(value => {
        ...
        if (dateFormatter) {
            text = text + dateFormatter(date, locale);
        } else if (value === '\'\'') {
            text = text + '\'';
        } else {
            text = text + value.replace(/(^'|'$)/g, '').replace(/''/g, '\'');
        }
...
/*
function formatDateTime(str: string, optValues: string[]) {
    if (optValues) {
